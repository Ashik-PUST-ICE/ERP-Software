...
class OrderProfitLossController extends Controller
{
        ...
        if ($request->ajax()) {
            return datatables(OrderProfitLoss::with('order')->latest('id'))
                ...
                ->addColumn('status', function ($row) {
                    [$label, $class] = garmentProfitLossStatuses()[$row->status] ?? ['Unknown', 'zBadge-warning'];
                    return '<div class="zBadge ' . $class . '">' . __($label) . '</div>';
                })
                ->addColumn('action', fn ($row) => '<div class="inline-flex"><div class="dropdown options-area"><a class="options-btn" href="#" data-bs-toggle="dropdown"><i class="fa-solid fa-ellipsis"></i></a><ul class="dropdown-menu dropdown-menu-end"><li><a class="dropdown-item" href="javascript:void(0)" onclick="getEditModal(\'' . route('admin.garments.profit-loss.edit', $row->id) . '\', \'#edit-modal\')">' . __('Edit') . '</a></li><li><a class="dropdown-item" href="javascript:void(0)" onclick="deleteItem(\'' . route('admin.garments.profit-loss.destroy', $row->id) . '\', \'garmentProfitLossDataTable\')">' . __('Delete') . '</a></li></ul></div></div>')
                ->rawColumns(['status', 'action'])
                ->make(true);
        }

        return view('admin.garments.profit-loss.index', [
            'title' => __('Order-wise Profit & Loss'),
            'orders' => GarmentOrder::latest()->get(),
            'activeGarments' => 'active',
            'activeGarmentProfitLoss' => 'active',
            'showGarmentsMenu' => 'show',
        ]);
    }

    public function store(OrderProfitLossRequest $request)
    {
        return $this->profitLossService->store($request);
    }

    public function edit($id)
    {
        $profitLoss = OrderProfitLoss::findOrFail($id);
        $orders = GarmentOrder::latest()->get();
        return view('admin.garments.profit-loss.form', compact('profitLoss', 'orders'));
    }

    public function update(OrderProfitLossRequest $request, $id)
    {
        $request->merge(['id' => $id]);
        return $this->profitLossService->store($request);
    }
    ...
    {
        return $this->profitLossService->destroy($id);
    }
}

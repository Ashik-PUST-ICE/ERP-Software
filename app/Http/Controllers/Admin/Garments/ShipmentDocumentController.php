...
class ShipmentDocumentController extends Controller
{
    ...
    {
        $document = ShipmentDocument::findOrFail($id);
        $orders = GarmentOrder::latest()->get();
        return view('admin.garments.shipment-documents.form', compact('document', 'orders'));
    }

    public function update(ShipmentDocumentRequest $request, $id)
    {
        $request->merge(['id' => $id]);
        return $this->documentService->store($request);
    }
    ...
    {
        return $this->documentService->destroy($id);
    }
}

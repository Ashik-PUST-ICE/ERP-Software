...
class GatewayService
{
    ...
    public function getCurrenciesByGatewayId($id)
    {
        ...
        if ($data['gateway']->slug == 'bank') {
            $data['banks'] = Bank::where('gateway_id', $data['gateway']->id)->where('status', ACTIVE)->get();
        }
    ...
    public function store($request)
    {
        ...
        try {
            ...
            if ($gateway->slug == 'bank' && isset($request->bank['name']) && !empty($request->bank['name'])) {
                $bankIds = [];
                for ($i = 0; $i < count($request->bank['name']); $i++) {
                    // Skip if bank name is empty
                    if (empty($request->bank['name'][$i])) {
                        continue;
                    }
                    $bank = Bank::updateOrCreate([
                        ...
                        'status' => $request->bank['status'][$i] ?? 0,
                    ]);

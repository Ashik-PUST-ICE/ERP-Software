...
(function ($) {
    ...
    $(document).on("change", ".ticket-status-change", function () {
        var route = $('#statusChangeRoute').val();
        var ticketId = $(this).data('ticket');
        var status = $(this).val();
        if (!route || !ticketId) return;
        $.ajax({
            type: 'GET',
            url: route,
    ...
    window.alertAjaxMessage = function (type, message) {
        ...
        if (!message || message === '') {
            console.warn('Empty message provided to alertAjaxMessage');
            return false;
        }

        if (type === 'success') {
            console.log(message + " This is success message");
    ...
    window.settingCommonHandler = function (data) {
        ...
        $('.error-message').remove();
        $('.is-invalid').removeClass('is-invalid');
        console.log('settingCommonHandler received data:', data);
        ...
        // Check if status is true (handle both boolean true and string "true")
        var status = responseData.status || responseData['status'];
        if (responseData && (status === true || status === 'true' || status === 1)) {
            output = responseData.message || responseData['message'] || 'Settings saved successfully';
            type = 'success';

            // Show toastr message using alertAjaxMessage
            alertAjaxMessage(type, output);

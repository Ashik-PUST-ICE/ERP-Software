    ...
    $(document).ready(function () {
        var table = $('#attendanceDataTable').DataTable({
            ...
            drawCallback: function () {
                ...
                if (!$wrap.length) {
                    $('#attendanceDataTable').after('<div id="attendance-pagination-wrap" class="d-flex justify-content-center mt-20 tablePagi"></div>');
        ...
        $('#searchData').off('keyup.hrmAttendance').on('keyup.hrmAttendance', function () {
            table.search(this.value).draw();
        });

        $('#filterDate, #filterStatus, #filterDepartment').change(function () {
            table.ajax.reload();
        });
    });

    window.markAttendance = function (empId, date) {

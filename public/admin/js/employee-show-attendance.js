(function ($) {
    ...
    $(document).ready(function () {
        var route = $('#attendance-history-route').val();
        var empId = $('#attendanceEmployeeId').val();
        console.log('attendance init', route, empId);
        if (!route || !empId) return;

        var table = $('#attendanceHistoryTable').DataTable({
            pageLength: 10,
            ordering: false,
            serverSide: true,
            processing: true,
            ...
            ajax: {
                ...
                data: function (d) {
                    d.employee_id = empId;
                    d.status = $('#attendanceFilterStatus').val();
                },
                error: function (xhr, error, thrown) {
                    console.error('attendance ajax error', xhr.status, thrown);
                }
            },
            dom: 't',
            columnDefs: [
            ...
            columns: [
                ...
                { data: 'check_in', name: 'check_in', searchable: false, orderable: false },
                { data: 'check_out', name: 'check_out', searchable: false, orderable: false },
            ...
            drawCallback: function () {
                ...
                var $wrap = $("#attendance-pagination-wrap");
                if (!$wrap.length) {
                    $('#attendanceHistoryTable').after('<div id="attendance-pagination-wrap" class="d-flex justify-content-center mt-20 tablePagi"></div>');
                    $wrap = $("#attendance-pagination-wrap");
                }
        ...
        $('#attendanceFilterStatus').change(function () {
            table.ajax.reload();
        });
    });

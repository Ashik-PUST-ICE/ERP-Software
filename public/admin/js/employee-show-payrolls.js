(function ($) {
    ...
    $(document).ready(function () {
        var route = $('#payroll-history-route').val();
        var empId = $('#payrollEmployeeId').val();
        console.log('payrolls init', route, empId);
        if (!route || !empId) return;

        var table = $('#payrollHistoryTable').DataTable({
            pageLength: 10,
            ordering: false,
            serverSide: true,
            processing: true,
            ...
            ajax: {
                ...
                data: function (d) {
                    d.employee_id = empId;
                    d.status = $('#payrollFilterStatus').val();
                },
                error: function (xhr, error, thrown) {
                    console.error('payrolls ajax error', xhr.status, thrown);
                }
            },
            dom: 't',
            columnDefs: [
            ...
            columns: [
                { data: 'payroll_month', name: 'payroll_month', responsivePriority: 1 },
                { data: 'basic_salary', name: 'basic_salary', searchable: false, orderable: false },
                { data: 'allowances', name: 'allowances', searchable: false, orderable: false },
                { data: 'deductions', name: 'deductions', searchable: false, orderable: false },
                { data: 'net_salary', name: 'net_salary', searchable: false, orderable: false },
            ...
            drawCallback: function () {
                ...
                var $wrap = $("#payroll-pagination-wrap");
                if (!$wrap.length) {
                    $('#payrollHistoryTable').after('<div id="payroll-pagination-wrap" class="d-flex justify-content-center mt-20 tablePagi"></div>');
                    $wrap = $("#payroll-pagination-wrap");
                }
        ...
        $('#payrollFilterStatus').change(function () {
            table.ajax.reload();
        });
    });

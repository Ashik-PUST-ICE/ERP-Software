(function ($) {
    ...
    function isVideoOnlyPostType() {
        ...
        // Reels, Shorts, and Video are video-only; Story can have both images and videos
    ...
    $(document).ready(function () {
        ...
        if ($cfg.length && $cfg.data('is-edit')) {
            try {
                var existingImgs = $cfg.data('existing-images');
                var existingVids = $cfg.data('existing-videos');
                ...
                selectedImages.forEach(function (img) {
                    $('.gallery-item-select[data-id="' + img.id + '"][data-type="image"]').addClass('selected').find('.select-icon').show();
                });
                selectedVideos.forEach(function (vid) {
                    $('.gallery-item-select[data-id="' + vid.id + '"][data-type="video"]').addClass('selected').find('.select-icon').show();
                });
                ...
                console.warn('campaign-config existing media parse error', e);
            }
        }
        ...
        (function initAIAssistant() {
            ...
            function optimizePrompt(prompt, contentType, platform, postType) {
                var platformSpecifics = {
                    'facebook': {
                        'Feed': 'Write engaging Facebook post content. Keep it conversational and shareable.',
                        'Reels': 'Create a short, catchy caption for a Facebook Reel video.',
                        'Story': 'Write a brief, casual message for a Facebook Story.'
                    },
                    'instagram': {
                        'Feed': 'Create an Instagram caption with line breaks, emojis, and hashtags.',
                        'Reels': 'Write an attention-grabbing caption for Instagram Reel.',
                        'Story': 'Create a short, fun message for Instagram Story.'
                    },
                    'twitter': {
                        'Feed': 'Write a concise Twitter post under 280 characters.'
                    },
                    'linkedin': {
                        'Feed': 'Create a professional LinkedIn post for industry insights.'
                    },
                    'tiktok': {
                        'Reels': 'Write a viral TikTok caption with trending language.'
                    },
                    'youtube': {
                        'Reels': 'Create a YouTube Shorts caption.',
                        'Shorts': 'Write a YouTube Shorts caption.',
                        'Video': 'Write a YouTube video caption. Make it engaging.'
                    },
                    'threads': {
                        'Feed': 'Create a Threads post with conversational tone.'
                    }
                };
                ...
                if (contentType === 'text') {
                    if (platformSpecifics[platform] && platformSpecifics[platform][postType]) {
                        ...
                        instructions = 'Create engaging social media content for ' + platform + '.';
                    }
                } else if (contentType === 'image') {
                    instructions = 'Generate an image for ' + platform + ' ' + postType + '. ';
                    if (platform === 'instagram') instructions += 'Make it aesthetic and trending.';
                    else if (platform === 'linkedin') instructions += 'Make it professional.';
                    else instructions += 'Make it visually striking.';
                    ...
                    .then(function (result) {
                        ...
                        if (result.ok && data && data.success) {
                            ...
                            if (contentType === 'text' && data.text) {
                                ...
                                showAiSuccess('Text generated for ' + platform + ' ' + postType + '!');
                                ...
                                showAiSuccess('Image generated for ' + platform + ' ' + postType + '! Saved to gallery.');
                                ...
                                showAiSuccess('Video generated for ' + platform + ' ' + postType + '! Saved to gallery.');
                            } else {
                                showAiError('Unexpected response from AI.');
                            }
                        } else {
                            showAiError((data && data.message) || 'AI generation failed.');
                        }
                    })
                    .catch(function () {
                        showAiError('Request failed. Try again.');
                    })
                    .finally(function () {
                        $('#ai-loading-spinner').hide();
            ...
            $(document).on('click', '#use-video-btn', function () {
                if (generatedContent.type === 'video' && generatedContent.mediaUrl) {
                    ...
                    if (typeof selectedVideos !== 'undefined') {
                        selectedVideos.push({
                            id: videoId,
                            url: generatedContent.mediaUrl
                        });
        ...
        if ($('#PostContent').length && $('.platform-filter').length) {
            ...
            function updatePostTypeTabs() {
                ...
                var postTypeInput = $('#campaign_post_type');
                if (postTypeInput.length) postTypeInput.val(postTypes[0]);
                syncContentToPreview();
                syncMediaToPreview();
            }
            ...
            $('.platform-filter').on('change', function () {
                if ($(this).is(':checked')) {
                    ...
                    selectedAccounts = [];
                    $('#selected_account_ids').val('');
                }

                var selectedPlatform = $('.platform-filter:checked').data('platform') || '';
                $('#selected_platform').val(selectedPlatform);
            ...
            var selectedPlatform = $('.platform-filter:checked').data('platform') || '';
            if (!selectedPlatform) {
                selectedPlatform = $('#selected_platform').val() || '';
            }
            if (selectedPlatform) {
                $('.platform-filter[data-platform="' + selectedPlatform + '"]').prop('checked', true);
                filterAccounts();
                updatePostTypeTabs();
            } else {
                filterAccounts();
                if ($('#PostContent').children().length === 0) {
                    var tabsContainer = $('#Post');
                    var tabContentContainer = $('#PostContent');
                    if (tabsContainer.length && tabContentContainer.length) {
                        tabsContainer.append('<li class="nav-item" role="presentation"><button class="nav-link active" id="Feed-tab" data-bs-toggle="tab" data-bs-target="#Feed-tab-pane" type="button" role="tab" aria-controls="Feed-tab-pane" aria-selected="true">Feed</button></li>');
                        tabContentContainer.append('<div class="tab-pane fade show active" id="Feed-tab-pane" role="tabpanel" aria-labelledby="Feed-tab" tabindex="0"><div class="feed-wrap"><div class="feed-top"><span class="icon"><i class="fa-brands fa-facebook-f"></i></span><div class="feed-top-info"><h4>Account</h4><span>Now </span></div></div><div class="feed-middle"><p class="template-preview-content">What\'s in your mind</p></div><div class="template-preview-media feed-media" style="display:none;"></div><div class="feed-bottom"><ul class="feed-action-lsit"><li><svg width="19" height="19" viewBox="0 0 19 19" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M1.5835 9.89583C1.5835 9.02136 2.29238 8.3125 3.16683 8.3125C4.4785 8.3125 5.54183 9.37579 5.54183 10.6875V13.8542C5.54183 15.1659 4.4785 16.2292 3.16683 16.2292C2.29238 16.2292 1.5835 15.5203 1.5835 14.6458V9.89583Z" stroke="#141B34" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" /><path d="M12.2538 6.18012L12.043 6.86102C11.8702 7.41896 11.7839 7.69793 11.8503 7.91825C11.9041 8.09646 12.022 8.25012 12.1828 8.35122C12.3815 8.47614 12.6821 8.47614 13.2833 8.47614H13.6031C15.6378 8.47614 16.6551 8.47614 17.1356 9.07844C17.1905 9.14724 17.2393 9.22047 17.2816 9.29726C17.6512 9.96891 17.231 10.8738 16.3905 12.6835C15.6192 14.3443 15.2335 15.1747 14.5174 15.6634C14.4482 15.7107 14.3769 15.7554 14.3039 15.7972C13.55 16.2293 12.616 16.2293 10.7478 16.2293H10.3426C8.07935 16.2293 6.94773 16.2293 6.24461 15.5481C5.5415 14.8668 5.5415 13.7703 5.5415 11.5774V10.8067C5.5415 9.65422 5.5415 9.07805 5.74602 8.55064C5.95054 8.02323 6.34216 7.58959 7.12538 6.7223L10.3644 3.13561C10.4456 3.04566 10.4863 3.00068 10.5221 2.96951C10.8564 2.67859 11.3723 2.71134 11.6646 3.04202C11.6959 3.07745 11.7301 3.12718 11.7985 3.22663C11.9056 3.3822 11.9591 3.45999 12.0058 3.53705C12.4233 4.22697 12.5497 5.04653 12.3584 5.82457C12.337 5.91148 12.3093 6.00108 12.2538 6.18012Z" stroke="#141B34" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" /></svg></li><li><svg width="19" height="19" viewBox="0 0 19 19" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M9.50344 9.5H9.51056M12.6665 9.5H12.6737M6.34033 9.5H6.34743" stroke="#141B34" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" /><path d="M17.0207 9.49984C17.0207 13.6535 13.6535 17.0207 9.49984 17.0207C8.21093 17.0207 6.9977 16.6964 5.93734 16.1251C4.45832 15.3281 3.46308 16.069 2.58536 16.2019C2.45221 16.2221 2.31961 16.1737 2.2244 16.0786C2.07987 15.934 2.05236 15.7105 2.13219 15.5224C2.47669 14.7104 2.793 13.1717 2.3617 11.8748C2.11343 11.1283 1.979 10.3297 1.979 9.49984C1.979 5.34619 5.34619 1.979 9.49984 1.979C13.6535 1.979 17.0207 5.34619 17.0207 9.49984Z" stroke="#141B34" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" /></svg></li><li><svg width="19" height="19" viewBox="0 0 19 19" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M12.6665 7.9165C13.7749 7.9165 14.3292 7.9165 14.7525 8.13223C15.1249 8.322 15.4277 8.62473 15.6174 8.99713C15.8332 9.42051 15.8332 9.97476 15.8332 11.0832V13.4582C15.8332 15.3241 15.8332 16.2571 15.2535 16.8368C14.6738 17.4165 13.7408 17.4165 11.8748 17.4165H7.12484C5.25886 17.4165 4.32588 17.4165 3.74619 16.8368C3.1665 16.2571 3.1665 15.3241 3.1665 13.4582V11.0832C3.1665 9.97476 3.1665 9.42051 3.38222 8.99713C3.57197 8.62473 3.87474 8.322 4.24714 8.13223C4.67051 7.9165 5.22473 7.9165 6.33317 7.9165" stroke="#141B34" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" /><path d="M9.5 12.6665V3.1665" stroke="#141B34" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" /><path d="M11.875 4.74998C11.875 4.74998 10.1258 2.37501 9.5 2.375C8.87411 2.37499 7.125 4.75 7.125 4.75" stroke="#141B34" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round" /></svg></li></ul></div></div></div>');
                        syncContentToPreview();
                        syncMediaToPreview();
                    }
                }
        ...
        $('.btn-next').on('click', function () {
            ...
            if (currentStep.find('input[required], textarea[required]').length > 0) {
                let isValid = true;
            ...
            if (nextStep.length) {
                ...
                const stepIndex = currentStep.index('.form-step');
                const nextStepIndex = nextStep.index('.form-step');

                $('.step').eq(stepIndex).removeClass('active');
                $('.step').eq(stepIndex + 1).addClass('active');

                if (nextStepIndex === 2) {
                    setTimeout(function () {
                        if (typeof window.updateReviewSection === 'function') {
                            window.updateReviewSection();
                        } else if (typeof updateReviewSection === 'function') {
                            updateReviewSection();
                        }
                    }, 300);
                }
        ...
        $('.account-checkbox').on('change', function () {
            ...
            $('#selected_account_ids').val(selectedAccounts.join(','));
        });
        ...
        $('#directMediaUpload').on('change', function (e) {
            ...
            if (videoOnly) {
                ...
                if (hasImage && hasVideo) {
                    if (typeof toastr !== 'undefined') toastr.warning('Feed allows multiple images OR one video, not both. Please select only images or only one video.');
                    $(this).val('');
                    return;
                }
                if (hasVideo) {
        ...
        window.updateReviewSection = function () {
            ...
            const platform = $('#selected_platform').val() || '';

            const reviewName = $('#review-name');
            const reviewStartDate = $('#review-start-date');
            const reviewEndDate = $('#review-end-date');
            const reviewPlatform = $('#review-platform');
            const reviewAccounts = $('#review-accounts');

            if (reviewName.length) {
                reviewName.text(campaignName || '-');
            }

            if (reviewStartDate.length) {
                if (startDate) {
                    reviewStartDate.text('Start Date: ' + startDate);
                } else {
                    reviewStartDate.text('-');
                }
            }

            if (reviewEndDate.length) {
                if (endDate) {
                    reviewEndDate.text('End Date: ' + endDate);
                } else {
                    reviewEndDate.text('-');
                }
            }

            if (reviewPlatform.length) {
                if (platform && platformLabels && platformLabels[platform]) {
                    reviewPlatform.text(platformLabels[platform]);
                } else if (platform) {
                    const platformText = platform.charAt(0).toUpperCase() + platform.slice(1);
                    reviewPlatform.text(platformText);
                } else {
                    reviewPlatform.text('-');
                }
            }
            ...
            checkedAccounts.each(function () {
                const $checkbox = $(this);
                const $label = $checkbox.closest('label.accounts-info');
                if ($label.length) {
                    const accountText = $label.clone().find('input, img').remove().end().text().trim();
                    if (accountText) {
                        accountNames.push(accountText);
                    }
                }
            });
            ...
            if (reviewAccounts.length) {
                if (accountNames.length > 0) {
                    if (accountNames.length === 1) {
                        reviewAccounts.html('<span>' + accountNames[0] + '</span>');
                    } else {
                        let accountsHtml = '<div style="display: flex; flex-direction: column; gap: 8px;">';
                        accountNames.forEach(function (name) {
                            accountsHtml += '<div class="account-area"><span>' + name + '</span></div>';
                        });
                        accountsHtml += '</div>';
                        reviewAccounts.html(accountsHtml);
                    }
                } else {
                    reviewAccounts.html('<span>-</span>');
                }
            }
        };

        var updateReviewSection = window.updateReviewSection;

        $(document).on('input change', '#CampaignName, #StartDate, #EndDate', function () {
            if ($('.form-step').eq(2).hasClass('active')) {
                setTimeout(function () {
                    if (typeof window.updateReviewSection === 'function') {
                        window.updateReviewSection();
                    }
        ...
        $(document).on('change', '.platform-filter, .account-checkbox', function () {
            if ($('.form-step').eq(2).hasClass('active')) {
                setTimeout(function () {
                    if (typeof window.updateReviewSection === 'function') {
                        window.updateReviewSection();
        ...
        (function initializeEditPageState() {
            const $accountIdsInput = $('#selected_account_ids');
            const $platformInput = $('#selected_platform');
            const $postTypeInput = $('#campaign_post_type');
            const $imageIdsInput = $('#selected_gallery_image_ids');
            const $videoIdsInput = $('#selected_gallery_video_ids');

            if (!$accountIdsInput.length && !$platformInput.length && !$imageIdsInput.length && !$videoIdsInput.length) {
                return;
            }

            if ($accountIdsInput.length && $accountIdsInput.val()) {
                const accountIds = $accountIdsInput.val().split(',');
                accountIds.forEach(function (id) {
                    if (!id) return;
                    $('.account-checkbox[value="' + id + '"]').prop('checked', true);
                    if (selectedAccounts.indexOf(id) === -1) {
                        selectedAccounts.push(id);
                    }
                });
                $accountIdsInput.val(selectedAccounts.join(','));
            }

            if ($platformInput.length && $platformInput.val()) {
                const platformLower = $platformInput.val().toLowerCase();
                $('.platform-filter[data-platform="' + platformLower + '"]').prop('checked', true);
                $('#selected_platform').val(platformLower);
                if (typeof filterAccounts === 'function') filterAccounts();
                if (typeof updatePostTypeTabs === 'function') updatePostTypeTabs();
            }

            setTimeout(function () {
                if ($postTypeInput.length && $postTypeInput.val()) {
                    const postType = $postTypeInput.val();
                    const $tab = $('#' + postType + '-tab');
                    if ($tab.length) {
                        $tab.click();
                    }
                }
            ...
            if (typeof syncMediaToPreview === 'function') {
                setTimeout(function () {
                    syncMediaToPreview();
                }, 300);
            }
        ...
        var campaignTable = $('#campaignTable').DataTable({
            ...
            columns: [
                { "data": "campaign_name", "name": "name", responsivePriority: 1 },
                { "data": "status", "name": "status", searchable: false, responsivePriority: 2 },
                { "data": "created_date", "name": "created_at", responsivePriority: 3 },
                { "data": "end_date", "name": "end_date", responsivePriority: 4 },
                { "data": "platform", "name": "platform", responsivePriority: 5 },
                { "data": "action", "name": "action", orderable: false, searchable: false, responsivePriority: 6 },
            ],
            drawCallback: function () {
                var info = campaignTable.page.info();
                var totalPages = info.pages;
                var currentPage = info.page + 1;
                var $wrap = $("#campaign-pagination-wrap");
                if (!$wrap.length) {
                    $('#campaignTable').after('<div id="campaign-pagination-wrap" class="d-flex justify-content-center mt-20 tablePagi"></div>');
                    $wrap = $("#campaign-pagination-wrap");
                }
                $wrap.html(renderGlobalPagination(totalPages, currentPage));
                $wrap.off("click", ".ajax-page").on("click", ".ajax-page", function (e) {
                    e.preventDefault();
                    var page = $(this).data("page");
                    if (page) campaignTable.page(page - 1).draw("page");
                });
            }
        });

        $('#searchData').off('keyup.adminCampaign').on('keyup.adminCampaign', function () {
            campaignTable.search($(this).val()).draw();
        });

        $(document).on('click', '.campaign-delete-btn', function (e) {
            e.preventDefault();
            var form = $(this).closest('form.campaign-delete-form');
        ...
        $(document).on('click', '.campaign-publish-btn', function (e) {
            e.preventDefault();
            var url = $(this).data('url');
            var campaignName = $(this).data('campaign-name') || 'Campaign';
            if (!url) return;
            var token = $('meta[name="csrf-token"]').attr('content');
            if (typeof Swal !== 'undefined') {
                Swal.fire({
                    title: 'Publish campaign posts?',
                    text: 'Publish all pending scheduled posts for "' + campaignName + '" now?',
                    icon: 'question',
                    showCancelButton: true,
                    ...
                    $.ajax({
                        ...
                        success: function (res) {
                            if (res.status) {
                                ...
                                if (typeof toastr !== 'undefined') toastr.error(res.message || 'Failed to publish');
                            }
                        },
                        error: function (xhr) {
                            var msg = (xhr.responseJSON && xhr.responseJSON.message) ? xhr.responseJSON.message : 'Failed to publish campaign posts.';
                            if (typeof toastr !== 'undefined') toastr.error(msg);
                        }
                    });
                });
                ...
                $.ajax({
                    ...
                    headers: { 'Accept': 'application/json', 'X-CSRF-TOKEN': token },
                    success: function (res) {
                        if (res.status && typeof toastr !== 'undefined') toastr.success(res.message);
                        if ($.fn.DataTable.isDataTable('#campaignTable')) {
                            $('#campaignTable').DataTable().ajax.reload(null, false);
                        }
                    },
                    error: function () {
                        if (typeof toastr !== 'undefined') toastr.error('Failed to publish campaign posts.');
                    }
                });
            }
        });
        ...
        $(document).on('submit', '#campaignForm', function (e) {
            ...
            var postType = ($('#campaign_post_type').val() || 'Feed').toLowerCase();
            if (postType === 'feed') {
            ...
            var isPublishNow = activeBtn && activeBtn.name === 'publish_now' && activeBtn.value === '1';
            var isSchedule = activeBtn && activeBtn.name === 'publish_now' && activeBtn.value === '0';

            if (isSchedule) {
                var scheduledTime = ($('#campaign_scheduled_time_field').val() || '').trim();
                if (!scheduledTime) {
                    e.preventDefault();
                    if (typeof toastr !== 'undefined') toastr.error('Please select a date and time using the calendar icon, then click "Schedule" in the picker before submitting.');
                    return false;
                }
            }

            if (isPublishNow) {
                var postLimitAllowed = $cfg.data('post-limit-allowed') || $cfg.data('postLimitAllowed') || '1';
                if (postLimitAllowed === '0' || postLimitAllowed === false) {
                    e.preventDefault();
                    var limitMsg = $cfg.data('post-limit-message') || $cfg.data('postLimitMessage') || 'You have reached your post limit. Please upgrade your package.';
            ...
            if (allowedProviders.length > 0) {
                var selectedPlatform = $('.platform-filter:checked').val() || '';
                if (selectedPlatform && allowedProviders.indexOf(selectedPlatform) === -1) {
                    e.preventDefault();
                    var platformName = selectedPlatform.charAt(0).toUpperCase() + selectedPlatform.slice(1);
            ...
            // Show loading overlay - all validations passed
            var isEdit = $('#campaign-config').length && $('#campaign-config').data('is-edit');


            var loadingTitle = isEdit ? 'Updating campaign' : 'Creating campaign';
            var loadingSubtitle = isEdit ? 'Please wait while we update your campaign.' : 'Please wait while we create your campaign.';

            showCreatePostLoading(loadingTitle, loadingSubtitle);
        });

        // Function to show loading overlay
        function showCreatePostLoading(title, subtitle) {
            // console.log('showCreatePostLoading function called with:', title, subtitle);
            ...
            var loadingHtml = '<div id="create-post-loading-overlay" style="position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,0.7);z-index:9999;display:flex;flex-direction:column;align-items:center;justify-content:center;">' +
                ...
                '<p style="margin:10px 0 20px;color:#666;font-size:14px;">' + subtitle + '</p>' +
            ...
            // console.log('Appending loading HTML to body');
            // Append overlay to body
            $('body').append(loadingHtml);

            // console.log('Showing overlay with fadeIn');
            // Show overlay

...
class TicketService
{
    ...
    public function ticketList($status)
    {
        $baseQuery = Ticket::leftJoin('users', 'tickets.client_id', '=', 'users.id')
            ...
            ->where('tickets.client_id', auth()->id())
        ...
        return datatables($data)
            ...
            ->editColumn('ticket_id', function ($row) {
                return getTicketIdHtml($row, 'admin');
            })
            ...
            ->addColumn('action', function ($row) {
                $detailsUrl = route('admin.ticket.details', encrypt($row->id));
                $editUrl = route('admin.ticket.edit', encrypt($row->id));
                $deleteUrl = route('admin.ticket.delete', encrypt($row->id));

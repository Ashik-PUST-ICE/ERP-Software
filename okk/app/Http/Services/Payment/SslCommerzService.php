...
class SslCommerzService extends BasePaymentService
{
    ...
    public function makePayment($amount)
    {
        ...
        // Order model removed - booking app deprecated
        // Use Payment model for subscription payments or use orderId directly
        $payment = null;
        if ($this->orderId) {
            $payment = PaymentModel::find($this->orderId);
        }
        
        $requestData = array();
        $requestData['tran_id'] = $payment ? $payment->id : ($this->orderId ?? uniqid()); // tran_id must be unique
        $requestData['product_category'] = "Payment for purchase";

        $user = auth()->user();
        $requestData['cus_name'] = $user->name ?? '';
        $requestData['cus_phone'] = $user->contact_number ?? '';
        $requestData['cus_email'] = $user->email ?? '';
        ...
        $requestData['ship_phone'] = $user->contact_number ?? '';
        $requestData['ship_country'] = ''; // Order customer removed - booking app deprecated

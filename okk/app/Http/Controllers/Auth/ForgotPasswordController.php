...
class ForgotPasswordController extends Controller
{
    ...
    */
    ...
    public function forgetVerify(Request $request, $token, $email)
    {
        $otp = $request->otp__field__1.$request->otp__field__2.$request->otp__field__3.$request->otp__field__4;
        $user = DB::table('password_resets')->join('users', 'users.email', '=', 'password_resets.email')->where('token', $token)->where('password_resets.email', $email)->select('users.email', 'users.name', 'password_resets.otp', 'password_resets.otp_expiry')->first();
    ...
    public function forgetVerifyResend(Request $request, $token, $email)
    {
        $user = DB::table('password_resets')->join('users', 'users.email', '=', 'password_resets.email')->where('token', $token)->where('password_resets.email', $email)->select('users.email', 'users.name', 'password_resets.otp', 'password_resets.otp_expiry')->first();
        if (getOption('email_verification_status', 0) == 1) {
            try {
                if (!($user->otp_expiry >= now())) {
                    ...
                    return redirect()->route('password.reset.verify_form', ['token' => $token, 'email' => $user->email])->with('success', __('We have sent a fresh verify email.'));
                }
                else{

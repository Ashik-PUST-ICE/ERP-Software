...
<div class="content-wrapper">
    <div class="section-title">
        <h2 class="title">{{ __('Edit Campaign') }}</h2>
    </div>
    <div class="section-wrap">
        <div class="form-wrapper">
            ...
            <form id="campaignForm" action="{{ route('admin.campaign.update', $campaign->id) }}" method="POST"
                enctype="multipart/form-data">
                @csrf
                @method('PUT')
                <input type="hidden" name="gallery_image_ids" id="selected_gallery_image_ids"
                    value="{{ $campaign->gallery_image_ids }}">
                <input type="hidden" name="gallery_video_ids" id="selected_gallery_video_ids"
                    value="{{ $campaign->gallery_video_ids }}">
                <input type="hidden" name="account_ids" id="selected_account_ids" value="{{ $campaign->account_ids }}">
                <input type="hidden" name="platform" id="selected_platform" value="{{ $campaign->platform }}">
                <input type="hidden" name="post_type" id="campaign_post_type"
                    value="{{ $campaign->post_type ?? 'Feed' }}">
                <input type="hidden" name="status" id="campaign_status" value="{{ in_array($campaign->status ?? '', ['pending','posted','failed']) ? $campaign->status : 'pending' }}">
                <input type="hidden" name="scheduled_time" id="campaign_scheduled_time_field"
                    value="{{ old('scheduled_time', $campaign->scheduled_time ? formatDateToCustomer($campaign->scheduled_time, 'Y-m-d H:i', 'UTC', getAppTimeZone()) : '') }}">
                                ...
                                <div class="col-lg-12">
                                    <div class="form-group">
                                        ...
                                        <input type="text" class="form-control" id="CampaignName" name="name"
                                            value="{{ old('name', $campaign->name) }}" placeholder="{{ __('Demo') }}"
                                            required>
                                    </div>
                                </div>
                                ...
                                <div class="col-lg-12">
                                    <div class="form-group">
                                        ...
                                        <textarea id="Description" name="description" class="summernote" rows="5"
                                            placeholder="{{ __('Enter campaign description...') }}">{{ old('description', $campaign->description) }}</textarea>
                                    </div>
                                </div>
                            </div>
                        </div>
                ...
                <div class="form-step">
                    ...
                    <div class="review-launch-area">
                        ...
                        <div class="ready-launch text-center mt-4">
                            ...
                            <p>{{ __('Your campaign is configured and ready to go. Click "Update Campaign" to save changes.') }}
                            </p>
                        </div>
                    </div>
                    <div class="actions">
                        <button type="button" class="btn-secondary primary-btn btn-back">{{ __('Back') }}</button>
                        <button type="submit" class="primary-btn" name="action" value="update"
                            id="updateCampaignBtn">{{ __('Update Campaign') }}</button>
                        <!-- <button type="submit" class="primary-btn" name="publish_now" value="1"
                            id="campaignPostBtn">{{ __('Post') }}</button> -->
                        <!-- <button type="submit" class="primary-btn btn-secondary" name="publish_now" value="0"
                            id="campaignScheduleBtn">{{ __('Schedule') }}</button> -->
                        {{-- Schedule date picker (same as create page) --}}
...
<div id="campaign-config" data-is-edit="true"
    data-existing-images="{{ json_encode(($existingMedia ?? [])['images'] ?? []) }}"
    data-existing-videos="{{ json_encode(($existingMedia ?? [])['videos'] ?? []) }}"
    ...
    data-post-limit-message="{{ $postLimitInfo['message'] ?? '' }}"
    style="display:none;"></div>

<!-- GalleryModal - Same as create -->
<div class="offcanvas offcanvas-end" tabindex="-1" id="GalleryModal" aria-labelledby="GalleryModalLabel">
    ...
    <div class="offcanvas-body">
        <!-- Same gallery content as create.blade.php -->
        ...
        <div class="tab-content" id="GalleryTabContent">
            ...
            <!-- Images and Videos tabs similar to create -->
        </div>
    </div>

...
<div class="section-title">
    <h2 class="title">{{ __('Edit Post') }}</h2>
</div>
<form action="{{ route('admin.all-posts.update', $post) }}" method="POST" id="scheduledPostForm"
    enctype="multipart/form-data">
    @csrf
    @method('PUT')
    <input type="hidden" name="publish_now" value="0" id="publish_now_field">
    <input type="hidden" name="scheduled_time" id="scheduled_time_field"
        value="{{ old('scheduled_time', $post->scheduled_time ? $post->scheduled_time->format('Y-m-d H:i') : '') }}">
    <input type="hidden" name="gallery_image_ids" id="selected_gallery_image_ids"
        value="{{ old('gallery_image_ids', $post->post_metadata['gallery_image_ids'] ?? '') }}">
    <input type="hidden" name="gallery_video_ids" id="selected_gallery_video_ids"
        value="{{ old('gallery_video_ids', $post->post_metadata['gallery_video_ids'] ?? '') }}">

    <div class="create-post-wrap">
        <div class="create-post-left">
            <div class="post-left-top">
                <ul class="post-latform-lsit">
                    @php $postPlatform = strtolower($post->socialMediaAccount->platform ?? ''); @endphp
                    ...
                    <li>
                        <label class="platform-name">
                            <input type="checkbox" class="platform-filter" value="{{ strtolower($platform) }}"
                                ...
                                {{ (strtolower($platform) === $postPlatform) ? 'checked' : '' }}>
                    ...
                    <li>
                        <label class="accounts-info">
                            <input class="form-check-input" type="checkbox" name="account_ids[]"
                                ...
                                {{ ($post->social_media_account_id == $account->id || (is_array(old('account_ids')) && in_array($account->id, old('account_ids')))) ? 'checked' : '' }}>
        ...
        <div class="create-post-center">
            <div class="post-center-top">
                ...
                <div class="post-content">
                    ...
                    <textarea class="post-content-box" name="content" id="content"
                        placeholder="Start from here...">{{ old('content', $post->content) }}</textarea>
                </div>
            </div>
            <input type="hidden" name="post_type" id="post_type"
                value="{{ old('post_type', ucfirst($post->post_type ?? 'feed')) }}">
            <div class="post-center-footer">
                                ...
                                <div id="custom-picker-box" class="picker-card shadow-sm" style="display:none;">
                                    ...
                                    <div class="picker-footer">
                                        <div class="scheduled-info">
                                            <p>Scheduled for: <strong id="display-date">Select a date</strong></p>
                    ...
                    <div class="post-footer-right">
                        <div class="btn-list mt-0">
                            <a href="javascript:void(0)" id="aiAssistantBtn" class="primary-btn btn-outline-secondary">
                                ...
                                AI Assistant</a>
                            <button type="submit" class="primary-btn">Update Post <svg width="15" height="15"
                ...
                <div class="save-template-area" style="display: none;"></div>
            </div>
        </div>
...
<div id="post-config" data-is-edit="true" data-post-type="{{ $postConfig['postType'] }}"
    data-platform="{{ $postConfig['platform'] }}" data-scheduled-time="{{ $postConfig['scheduledTime'] }}"
    data-gallery-image-ids="{{ $postConfig['galleryImageIds'] }}"
    data-gallery-video-ids="{{ $postConfig['galleryVideoIds'] }}" data-template-list-url="{{ $templateListUrl }}"
    data-hashtags-url="{{ $hashtagsIndexUrl }}" data-platform-key-map='@json($platformKeyToName)'

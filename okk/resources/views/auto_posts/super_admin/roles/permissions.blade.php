        ...
        <div class="section-wrap">
            <div class="section-inner-title">
                <div class="d-flex justify-content-between align-items-center">
                    <h3 class="title">{{ $role->display_name }} - {{ __('Permissions') }}</h3>
                    <a href="{{route('super_admin.roles.index')}}" class="primary-btn btn-secondary">
                        <i class="fa fa-arrow-left me-2"></i>{{ __('Back') }}
                    </a>
                </div>
            </div>

            <form data-handler="commonResponse" action="{{route('super_admin.roles.update.permissions', [$role->id])}}"
...
@endsection
@push('script')
<script src="{{ asset('super_admin/js/roles-permissions.js') }}"></script>
@endpush
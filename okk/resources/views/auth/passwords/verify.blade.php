...
<div class="auth-area">
    <div class="auth-left">
        <div class="auth-form-wrap">
            ...
            <div class="auth-top">
                <h2>{{ __('Confirm Reset Password') }}</h2>
                <h3>{{ __('Please check your email') }} <span>{{ $resetPassword->email }}</span>
                    {{ __('and enter the otp below to reset') }}</h3>
            </div>
            <form action="{{ route('password.reset.verify', ['token' => $token, 'email' => $resetPassword->email]) }}"
                ...
                <div class="auth-form">
                    ...
                    <button id="verify-btn" type="submit" class="auth-form-btn">{{ __('Continue') }}</button>
                </div>
            </form>
            <form method="POST"
                action="{{ route('password.reset.verify_resend', ['token' => $token, 'email' => $resetPassword->email]) }}"
...
var countDownDate = new Date('{{ $resetPassword->otp_expiry }}').getTime();

<form class="ajax reset" action="{{ route('admin.hrm.departments.update', $department->id) }}" method="post"
    data-handler="commonResponseWithPageLoad">
    @method('put')
    @csrf
    <div class="modal-body zModalTwo-body">
        <div class="d-flex justify-content-between align-items-center mb-4">
            <h4 class="fs-20 fw-500 lh-38 text-1b1c17 mb-0">{{ __('Edit Department') }}</h4>
            <div class="mClose">
                ...
                <div class="col-12">
                    <div class="form-group">
                        ...
                        <select class="form-control" name="status" required>
                            <option value="{{ STATUS_ACTIVE }}" {{ $department->status == STATUS_ACTIVE ? 'selected' : '' }}>{{ __('Active') }}</option>
                            <option value="{{ STATUS_DEACTIVATE }}" {{ $department->status == STATUS_DEACTIVATE ? 'selected' : '' }}>{{ __('Deactivate') }}</option>
                        </select>
                    </div>
                </div>
            </div>

            ...
            <div class="table-waraper">
                <div class="d-flex flex-wrap align-items-center justify-content-between gap-3 mb-4">
                    <div class="search-input-wrap mb-0 flex-grow-1" style="max-width: 380px;">
                        ...
                        <input type="text" class="search-input" id="searchData" placeholder="{{ __('Search costing variance...') }}" />
                    </div>
                        ...
                        <div class="select-wrap" style="min-width: 170px;">
                            <select class="form-select form-control sf-select wide" id="costingVarianceOrderFilter" style="height: 42px; border-radius: 10px; font-size: 13px;">
                                ...
                                @foreach($orderOptions as $option)
                                    <option value="{{ $option->id }}" @selected((int) request('order_id') === $option->id)>{{ $option->order_number }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                </div>
                <input type="hidden" id="costing-variance-route" value="{{ route('admin.garments.costing-variance.index') }}">
...
@push('script')
<script src="{{ asset('admin/js/garment-costing-variance.js') }}?ver={{ env('VERSION', 0) }}"></script>
@endpush

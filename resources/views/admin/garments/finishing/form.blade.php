                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control" name="order_id" required>
                            ...
                            @foreach($orders as $order)
                                <option value="{{ $order->id }}" @selected(old('order_id', $finishing?->order_id) == $order->id)>{{ $order->order_number }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                ...
                <div class="col-md-6">
                    <div class="form-group">
                        ...
                        <select class="form-control" name="status" required>
                            @foreach(garmentFinishingStatuses() as $value => $status)
                                <option value="{{ $value }}" @selected(old('status', $finishing?->status ?? GARMENT_FINISHING_STATUS_PENDING) == $value)>{{ __($status[0]) }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>

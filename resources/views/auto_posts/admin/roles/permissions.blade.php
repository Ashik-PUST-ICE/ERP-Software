...
<style>
    ...
    .rp-page .rp-title { margin:3px 0 0; color:var(--rp-ink); font-size:22px; font-weight:700; }
    .rp-page .rp-subtitle { color:var(--rp-muted); font-size:13px; }
    ...
    .rp-page .rp-module { display:flex; align-items:center; justify-content:space-between; width:100%; margin:3px 0; padding:11px 12px; border:0; border-radius:10px; color:#5d6b7b; background:transparent; text-align:left; font-size:13px; transition:.2s; }
    ...
    .rp-page .rp-content-head h3 { margin:0 0 4px; color:var(--rp-ink); font-size:18px; font-weight:700; }
    .rp-page .rp-content-head p { margin:0; color:var(--rp-muted); font-size:12px; }
    ...
    .rp-page .rp-section-title h4 { margin:0; color:var(--rp-ink); font-size:15px; font-weight:700; }
    ...
    .rp-page .rp-permission span { color:#344054; cursor:pointer; font-size:12px; line-height:1.45; }
...
$(function () {
    ...
    $('#permission-search').on('input', function () {
        ...
        $('.permission-item').each(function () { const same = String($(this).closest('.module-content').data('module')) === active; $(this).toggle(same && (!value || $(this).find('label').text().toLowerCase().includes(value))); });
    });
    $('#clear-permission-search').on('click', function () { $('#permission-search').val('').trigger('input').focus(); });
    $('.module-select-all').on('click', function () { const module = String($(this).data('module')); const boxes = $('.module-content').filter(function () { return String($(this).data('module')) === module; }).find('.permission-checkbox'); boxes.prop('checked', boxes.filter(':checked').length !== boxes.length); updateCount(); });

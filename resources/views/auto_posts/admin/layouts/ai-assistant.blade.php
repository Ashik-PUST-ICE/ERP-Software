<div id="ai-assistant" class="ai-assistant" data-history-url="{{ route('admin.ai.chat.history') }}"
    data-message-url="{{ route('admin.ai.chat.message') }}" data-csrf="{{ csrf_token() }}">
    <button type="button" class="ai-assistant-launcher" aria-label="{{ __('Open AI Assistant') }}" aria-expanded="false">
        <i class="fa-solid fa-sparkles"></i>
        <span class="ai-assistant-launcher-label">{{ __('AI Assistant') }}</span>
    </button>
    <section class="ai-assistant-panel" aria-label="{{ __('AI Assistant') }}" hidden>
        <header class="ai-assistant-header">
            <div>
                <strong>{{ __('AI Assistant') }}</strong>
                <small>{{ __('Ask about using your ERP') }}</small>
            </div>
            <button type="button" class="ai-assistant-close" aria-label="{{ __('Close') }}">&times;</button>
        </header>
        <div class="ai-assistant-messages" aria-live="polite">
            <div class="ai-assistant-empty">{{ __('Hi! How can I help you today?') }}</div>
        </div>
        <form class="ai-assistant-form">
            <textarea name="message" rows="1" maxlength="6000" placeholder="{{ __('Type your question...') }}" required></textarea>
            <button type="submit" aria-label="{{ __('Send') }}"><i class="fa-solid fa-paper-plane"></i></button>
        </form>
        <div class="ai-assistant-note">{{ __('AI can make mistakes. Check important information.') }}</div>
    </section>
</div>
...
(function () {
    ...
    form.addEventListener('submit', async event => {
        ...
        waiting.innerHTML = '<span class="ai-assistant-typing" aria-label="{{ __('Thinking...') }}"><span></span><span></span><span></span></span>';
        ...
        try {
            ...
            if (!response.ok || !data.success) {
                throw new Error(data.message || '{{ __('The server could not process your message. Please try again.') }}');
            }
            waiting.remove();
            conversationId = data.conversation_id || conversationId;
            addMessage('assistant', data.message);
        } catch (error) {
            waiting.textContent = error.message || '{{ __('Unable to get a response. Please try again.') }}';
